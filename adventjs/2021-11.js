...
export default function shouldBuyFidelity(times) {
  ...
  const priceWithoutFidelity = TICKET * times; 

  let priceWithFidelity = FIDELITY; 

  for (let i = 1; i <= times; i++) {
    const currentPrice = TICKET * Math.pow(DISCOUNT, i)   ;
    priceWithFidelity += currentPrice; 
  }; 

  return priceWithFidelity < priceWithoutFidelity ? true : false;
}; 
